...
const InputField = ({ field, label, icon: Icon, formData, handleChange }) => {
  ...
  const getInputClasses = (isTextArea = false) => {
    const baseClasses = `
      w-full p-4 rounded-xl dark:bg-white/10 bg-black/10 dark:text-white text-black placeholder-transparent 
    ...
    const hoverFocusClasses = isFocused
      ? "dark:shadow-[0_4px_12px_rgba(99,102,241,0.4)] border-[#6366f1]"
      : "dark:border-white/20 border-black/20 hover:border-[#6366f1] transition-colors duration-500";
  ...
  return (
    <div className="relative w-full group">
      {/* Icon and Label */}
      <div className="absolute left-4 top-4 flex items-center space-x-2 dark:text-gray-400 text-gray-600 transition-colors group-hover:text-[#6366f1]">
        <Icon className="w-5 h-5" />
        <label
          htmlFor={field}
          className={`
            absolute left-12 top-1/2 transform -translate-y-1/2 dark:text-gray-400 text-gray-600 text-sm transition-all duration-300 
